...
function FireTwitUI() {
  ...
  this._firetwit = new FireTwit("https://firetwit.firebaseio.com/");
  this._unload = null;
  ...
  self._firetwit.onStateChange(function(user) {
    self.onLoginStateChange(user);
  });
}

FireTwitUI.prototype._setupHandlers = function() {
  var self = this;
...
FireTwitUI.prototype._go = function(url) {
  window.History.pushState(null, null, url);
};

FireTwitUI.prototype._pageController = function(url) {
...
FireTwitUI.prototype._postHandler = function(e) {
  ...
  self._firetwit.post(sparkText.val(), function(err, done) {
    if (!err) {
...
FireTwitUI.prototype._handleNewSpark = function(listId, limit, func) {
  var self = this;
  func(
    limit,
...
FireTwitUI.prototype._formatDate = function(date) {
...
FireTwitUI.prototype._editableHandler = function(id, value) {
  if (id == "inputLocation") {
    this._firetwit.setProfileField("location", value);
  }
  if (id == "inputBio") {
    this._firetwit.setProfileField("bio", value);
  }
  return true;
};

FireTwitUI.prototype.onLoginStateChange = function(info) {
  this._spinner.stop();
  this._loggedIn = info;
...
FireTwitUI.prototype.logout = function(e) {
  if (e) {
    e.preventDefault();
  }
  this._firetwit.logout();
  this._loggedIn = false;
  this.renderHome();
};

FireTwitUI.prototype.render404 = function() {
  // TODO: Add 404 page.
  this.renderHome();
};

FireTwitUI.prototype.goHome = function() {
  this._go("/");
};

FireTwitUI.prototype.renderHome = function(e) {
  if (e) {
    e.preventDefault();
  }
  if (this._loggedIn) {
  ...
  self._handleNewSpark(
    "spark-index-list", 5,
    self._firetwit.onLatestSpark.bind(self._firetwit)
  );
  return function() { self._firetwit.unload(); };
};

FireTwitUI.prototype.renderSearch = function() {
  var self = this;
  ...
  self._firetwit.startSearch(function(results) {
  ...
  var onCharChange = function() {
    ...
    self._firetwit.updateSearchTerm(searchTerm);
  };
...
FireTwitUI.prototype.renderTimeline = function(info) {
  var self = this;
  $("#header").html(Mustache.to_html($("#tmpl-page-header").html(), {user: self._loggedIn}));
  ...
  self._handleNewSpark(
    ...
    self._firetwit.onNewSpark.bind(self._firetwit)
  );

  // Get some "suggested" users.
  self._firetwit.getSuggestedUsers(function(userid, info) {
    info.id = userid;
    ...
    button.click(function(e) {
      ...
      e.preventDefault();
      $button.remove();
      self._firetwit.follow(id, function(err, done) {
...
FireTwitUI.prototype.renderProfile = function(uid) {
  var self = this;
  ...
  // Update user info.
  self._firetwit.getUserInfo(uid, function(info) {
    info.id = uid;
    ...
    if (self._loggedIn && self._loggedIn.id != info.id) {
      button.click(function(e) {
        ...
        self._firetwit.follow(clickedButtonId, function(err, done) {
          // TODO FIXME: Check for errors!
  ...
  self._handleNewSpark(
    ...
    self._firetwit.onNewSparkFor.bind(self._firetwit, uid)
  );
  return function() { self._firetwit.unload(); };
};

FireTwitUI.prototype.renderSpark = function(id) {
  var self = this;
  $("#header").html(Mustache.to_html($("#tmpl-page-header").html(), {user: self._loggedIn}));

  // Render spark page body.
  self._firetwit.getSpark(id, function(spark) {
    if (spark !== null && spark.author) {
      self._firetwit.getUserInfo(spark.author, function(authorInfo) {
